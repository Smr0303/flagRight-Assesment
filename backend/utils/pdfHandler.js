...
const generatePDF = async (summary) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Summary Box
            doc.rect(50, 120, doc.page.width - 100, 60)
            ...
            doc.fillColor('#1e3a8a')
               ...
               .text(`Total Volume: $${(summary.totalVolume || 0).toFixed(2)}`, 270, 150);

            // Table Header
            const drawTableHeader = () => {
                doc.rect(table.x, table.y, 
                    table.columnWidth.id + table.columnWidth.amount + 
                    table.columnWidth.type + table.columnWidth.date, 
                    table.rowHeight)
                   .fill('#2563eb');
            ...
            const drawTableRow = (transaction, rowIndex) => {
                ...
                // Draw cell content
                let xOffset = table.x + 10;
                doc.text(transaction.transactionId.substring(0, 12) + '...', 
                    xOffset, yPos + 10);

                xOffset += table.columnWidth.id;
                doc.text(`${transaction.amount.toFixed(2)} ${transaction.currency}`, 
                    xOffset, yPos + 10);
            ...
            if (summary.top4Transactions && summary.top4Transactions.length > 0) {
                ...
                doc.fillColor('#334155')
                   ...
                   .text('No transactions available', 
                        table.x, table.y + table.rowHeight + 10);
            }

            // Detailed Transaction Section
            if (summary.top4Transactions && summary.top4Transactions.length > 0) {
                const detailsStartY = table.y + 
                    ((summary.top4Transactions.length + 1) * table.rowHeight) + 50;
                ...
                summary.top4Transactions.forEach((transaction, index) => {
                    ...
                    // Detail box
                    doc.rect(table.x, yPos, doc.page.width - 100, 100)
                       .fillAndStroke('#f8fafc', '#e2e8f0');
                    ...
                    doc.text(`Transaction ID: ${transaction.transactionId}`, 
                        leftColumn, yPos + 15)
                       .text(`Origin User: ${transaction.originUser}`, 
                        leftColumn, yPos + 35)
                       .text(`Origin Country: ${transaction.originCountry}`, 
                        leftColumn, yPos + 55)
                    ...
                    doc.text(`Amount: ${transaction.amount} ${transaction.currency}`, 
                        rightColumn, yPos + 15)
                       .text(`Destination User: ${transaction.destinationUser}`, 
                        rightColumn, yPos + 35)
                       .text(`Destination Country: ${transaction.destinationCountry}`, 
                        rightColumn, yPos + 55);
                });
            }
...
module.exports ={
    getTransactionData, generatePDF
}